...
__device__ __forceinline__ int64_t page_buffer_offset_asymmetric(
    const int k_or_v, const int token_idx, const int scalar_offset,
    ...
    if (k_or_v == 0) {  
        // Key: [num_blocks, num_heads, block_size, head_size]
        return block_idx * num_heads * block_size * head_size +
               head_idx * block_size * head_size +
               block_offset * head_size + 
               head_offset;
    } else {
        // Value: [num_blocks, num_heads, head_size, block_size]
        return block_idx * num_heads * head_size * block_size +
...
template <typename scalar_t, bool DIRECTION>
__global__ void load_and_reshape_multi_layer_kernel_asymmetric(
    scalar_t* __restrict__ key_value,           // [2, num_layer, num_tokens, scalars_per_token]
    scalar_t** __restrict__ key_page_buffer_ptrs,  // [num_layers] * [2, ...]
    scalar_t** __restrict__ value_page_buffer_ptrs,  // [num_layers] * [2, ...]
    const int64_t* __restrict__ slot_mapping,   // [num_tokens]
    const int num_tokens, const int num_layers,
    const int num_heads, const int head_size, const int block_size) {
    ...
    const int64_t slot_idx = slot_mapping[token_id];
    scalar_t* key_page_buffer_ptr = key_page_buffer_ptrs[layer_id];
    scalar_t* value_page_buffer_ptr = value_page_buffer_ptrs[layer_id];
    ...
    for (int i = tid; i < n; i += num_threads) {
        const int64_t lmcache_offset =
            key_value_offset(k_or_v, layer_id, token_id, i, n,
                             num_tokens, num_layers);

        const int64_t vllm_offset = page_buffer_offset_asymmetric(
            k_or_v, slot_idx, i, num_heads, head_size, block_size);

        if (DIRECTION) { // 1 is paged buffer to LMCache
            if (k_or_v == 0) {
              key_value[lmcache_offset] = key_page_buffer_ptr[vllm_offset];
            } else {
              key_value[lmcache_offset] = value_page_buffer_ptr[vllm_offset];
            }
        } else { // 0 is LMCache to paged buffer
            if (k_or_v == 0) {
              key_page_buffer_ptr[vllm_offset] = key_value[lmcache_offset];
            } else {
              value_page_buffer_ptr[vllm_offset] = key_value[lmcache_offset];
            }
        }
    }
}
...
void multi_layer_kv_transfer_asymmetric(
...
//     printf("key_value dtype:: %s\n", c10::toString(dtype));
    ...
    // 使用 AT_DISPATCH_ALL_TYPES 宏
    AT_DISPATCH_ALL_TYPES_AND(at::ScalarType::Half, dtype, "multi_layer_kv_transfer_asymmetric", [&] {
        using scalar_t = scalar_t;

        scalar_t* key_value_ptr = get_kernel_ptr<scalar_t, torch::Tensor>(key_value);
        scalar_t** key_page_buffer_ptrs = 
            get_kernel_ptr<scalar_t*, const torch::Tensor>(key_cache_ptrs);
        scalar_t** value_page_buffer_ptrs = 
            get_kernel_ptr<scalar_t*, const torch::Tensor>(value_cache_ptrs);
        ...
        int num_layers = key_value.size(1);
        int num_tokens = slot_mapping.size(0);
        // printf("num_tokens (slot_mapping.size(0)) = %d\n", slot_mapping.size(0));
        ...
        if (not direction) {
            lmc::load_and_reshape_multi_layer_kernel_asymmetric<scalar_t, false>
                    <<<grid, block, 0, stream>>>(
                        key_value_ptr, key_page_buffer_ptrs, value_page_buffer_ptrs,
                        slot_mapping_ptr, num_tokens,
                        num_layers, num_heads, head_size, block_size);
            C10_CUDA_KERNEL_LAUNCH_CHECK();
        } else {
            lmc::load_and_reshape_multi_layer_kernel_asymmetric<scalar_t, true>
                    <<<grid, block, 0, stream>>>(
                        key_value_ptr, key_page_buffer_ptrs, value_page_buffer_ptrs,
                        slot_mapping_ptr, num_tokens,
                        num_layers, num_heads, head_size, block_size);
...
void lmcache_memcpy_async(uintptr_t dest, uintptr_t src, size_t nbytes,
  ...
  cudaMemcpyKind kind = (direction == 0)
